...
function ActivityRow({ session, language }: { session: ActivitySession; language: Language }) {
...
function Overview({ snapshot, language, showActivity }: { snapshot: DashboardSnapshot; language: Language; showActivity: () => void }) {
  ...
  return <>
    ...
    <section className="glass-card activity-card"><div className="card-heading"><div><span className="eyebrow">{text(language, '实时', 'LIVE')}</span><h3>{text(language, 'Codex 运行状态', 'Codex activity')}</h3></div><button className="text-button" onClick={showActivity}>{text(language, '查看全部', 'View all')}<ChevronRight size={14} /></button></div>{activities.length ? activities.map((session) => <ActivityRow session={session} language={language} key={session.sessionId} />) : <div className="empty-inline"><div className="activity-orb"><span /></div><div><strong>{text(language, '当前没有 Codex 任务', 'No active Codex task')}</strong><span>{text(language, '在设置中启用实时状态 Hook 后，活动会显示在这里。', 'Enable the live status Hook in Settings to see activity here.')}</span></div></div>}</section>
...
function ActivityPage({ snapshot, language }: { snapshot: DashboardSnapshot; language: Language }) {
  return <section className="glass-card activity-card full-card"><div className="card-heading"><div><span className="eyebrow">{text(language, '会话状态机', 'SESSION STATE MACHINE')}</span><h3>{text(language, '最近的 Codex 活动', 'Recent Codex activity')}</h3></div><span className={`soft-chip ${snapshot.health.activityHook === 'receiving' ? 'live-chip' : ''}`}>{healthLabel(snapshot.health.activityHook, language)}</span></div>{snapshot.activities.length ? snapshot.activities.map((session) => <ActivityRow session={session} language={language} key={session.sessionId} />) : <div className="large-empty"><Activity size={30} /><h3>{text(language, '等待活动', 'Waiting for activity')}</h3><p>{text(language, '在设置中启用本地 Hook，可实时查看思考、工具调用、批准、上下文压缩和子代理状态。', 'Enable the local Hook in Settings to see thinking, tools, approvals, compaction and subagents in real time.')}</p></div>}</section>
}
...
function SettingsPage({ snapshot, language, setLanguage, theme, setTheme }: { snapshot: DashboardSnapshot; language: Language; setLanguage: (language: Language) => void; theme: Theme; setTheme: (theme: Theme) => void }) {
  ...
  return <section className="settings-stack">
    ...
    <article className="glass-card settings-card"><span className="eyebrow">{text(language, '实时监控', 'LIVE MONITORING')}</span><h3>{text(language, 'Codex 运行状态', 'Codex activity')}</h3>
      <div className="hook-row"><span><strong>{healthLabel(snapshot.health.activityHook, language)}</strong><small>{text(language, '仅发送事件、会话标识、项目目录和工具名称，不发送提示词或工具参数。', 'Only events, session IDs, project paths and tool names are sent locally.')}</small></span><button className="primary-button" disabled={installing || snapshot.health.activityHook === 'receiving'} onClick={() => void enableHook()}>{installing ? text(language, '安装中…', 'Installing…') : text(language, '启用实时状态', 'Enable live status')}</button></div>
      {(hookResult || snapshot.health.activityHook === 'pending-trust') && <p className="hook-notice">{text(language, 'Hook 已写入。请重启 Codex，在 Codex 中输入 /hooks 并信任 “CodePulse activity monitor”。', 'Hook installed. Restart Codex, run /hooks, and trust “CodePulse activity monitor”.')}</p>}
    </article>
